...
resources_merge <- resources_views%>%
  select(hhid_deid, mod_f_respondent_sex, trees_plant:trees_other_EN,
         land_kind_a, land_kind_c, clear_where_a, clear_where_c, clear_protect, honey, mush, insects,
...
final_agriculture%>%
  group_by(clear_any)%>%
  ggplot(aes(as_factor(crops_1), fill = clear_any))+
  geom_bar()

#100 PERCENT BUGGY NEED TO FIX START HERE NEXT TIME YOU OPEN

#define response variable
y <- final_agriculture$clear_where_a

...
theme_usaid <- function(){ 
    ...
    theme(
      ...
      aspect.ratio = .9,
      #text elements
      plot.title = element_text(family = font, size = 14, hjust = 0, vjust = 1),
...
#This graph is also below.  Kept here for immediate feedback on theme changes.

final_member_compact%>%
  mutate(probability_not_in_poverty = (100-ppi)/100)%>%
  mutate(how_food_secure = 1-fies_unw_avg)%>%
...
#  geom_point()+
  ...
  labs(title = "Correlation between Poverty Probability and Food Insecurity\nby Group Membership", x = "Probability Household Is Not in Poverty", y = "Level of Food Security (Reverse FIES Score)")+
...
#  geom_point(position = 'jitter')+
  ...
  theme_usaid()+
  scale_color_manual(values = c("#BA0C2F", "#002F6C"), name = "", labels = c("Not a Member", "Member of a Group"), guide = guide_legend(reverse = TRUE))+
  labs(title = "Correlation between Environmental Importance Perception and\nNatural Resource Management Practice Use by Group Membership", x = "Perceived Importance of Ecosystem Services Score", y = "Number of Natural Resource Management Practices Used")

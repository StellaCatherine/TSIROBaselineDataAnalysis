...
final_member_compact <- final_member%>%
  select(member_sum, member_any, threats, threats_sum, protect_sum, nrm_pract_sum, trees_plant, clear_where_a, clear_where_b, clear_where_c, clear_protect, loss, fies_unw_avg, mddw, ppi)

final_member_compact %>%
  drop_na(.)%>%
  gather(threats, threats_sum, protect_sum, nrm_pract_sum, trees_plant, clear_where_a, clear_where_b, clear_where_c, clear_protect, loss, fies_unw_avg, mddw, key = "var", value = "value") %>% 
...
final_member_compact %>%
  drop_na(ppi)%>%
  ggplot(aes(x = as_factor(member_any), fill = as_factor(ppi))) +
    geom_bar(position = 'fill') +
    theme_bw()
...
final_trees <- final_member%>%
  select(member_forest, threats_illegal_logging, wood, wood_access, fuel_sources_3, clear_where_a, land_kind_a, nrm_pract_c, trees_why_perm, trees_why_cut)%>%
  replace(is.na(.), 0)%>%

...
agriculture_merge <- agriculture%>%
  select(hhid_deid, nrm_pract:nrm_pract_g)
credit_merge <- credit
...
final <- narrow %>%
  mutate(nrm_pract_sum = rowSums(select(narrow, nrm_pract_a:nrm_pract_g)), .after = nrm_pract_g)%>%
  select(-nrm_pract, -head_sex.y, -spouse_sex.y)%>%

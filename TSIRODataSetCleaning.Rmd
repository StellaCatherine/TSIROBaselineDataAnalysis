...
all_test_replace <- list(
  remove_all_nas = ~na_if(.x,0),
  conversion = ~as.character(.x),
  mtest = ~str_replace(.x, "1","M"),
  ftest = ~str_replace(.x, "2","F"),
  itest = ~str_replace(.x, "3","I"),
  otest = ~str_replace(.x, "4","O")
)

credit_gender_alt <- credit_gender_b%>%
  as_tibble()%>%
  mutate(across(borrow_ngo_self:borrow_ngo_outside), na_if(.,0))%>%
  mutate(across(borrow_ngo_self:borrow_ngo_outside), convert(chr(.)))%>%
  mutate(across(borrow_ngo_self:borrow_ngo_outside), str_replace(.,c("1","2","3","4"),c("M","F","I","O")))%>%
  unite(borrow_ngo_gender, borrow_ngo_self:borrow_ngo_outside, sep = "")

credit_gender_intermediate <- credit_gender_b%>%
  select(borrow_ngo_self:borrow_ngo_outside)%>%
  mutate_all(~na_if(., 0))%>%
  mutate_all(as.character)%>%
  mutate_all(funs(str_replace(., "1","M")))%>%
  mutate_all(funs(str_replace(., "2","F")))%>%
  mutate_all(funs(str_replace(., "3","I")))%>%
  mutate_all(funs(str_replace(., "4","O")))%>%
  unite(borrow_ngo_gender, borrow_ngo_self, borrow_ngo_spouse, borrow_ngo_inside, borrow_ngo_outside, na.rm = TRUE, sep = "")

view(credit_gender_intermediate)
```

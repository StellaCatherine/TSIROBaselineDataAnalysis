...
resources_views <- resources_views %>%
  rename("threats_big_a" = "threats_illegal_logging",
         "threats_big_b" = "",
         "threats_big_c" = "",
         "threats_big_d" = "")
...
resources_merge <- resources_views%>%
  select(hhid_deid, mod_f_respondent_sex, trees_plant:trees_other_EN,
         land_kind_a, clear_where_a, clear_protect, honey, mush, insects,
         fruits, meat, plants, wood, char, fish, protect_water:loss_affect,
         threats:threats_big_i)
energy_merge <- energy%>%
  select(hhid_deid, starts_with("fuel_sources"), starts_with("fuel_men"), starts_with("fuel_women"))
poverty_merge <- poverty
...
narrow <- reduce(list(roster_merge, agriculture_merge, resources_merge, energy_merge, poverty_merge, foodsec_merge, empower_merge), full_join, by = "hhid_deid")
...
final%>%
  group_by(mod_f_respondent_sex)%>%
  summarise(across(protect_air:protect_avg, ~ (mean(.x, na.rm = TRUE))))%>%
  pivot_longer(protect_air:protect_avg, names_to = "protect_resource", names_prefix = "protect_")%>%
  ggplot(aes(protect_resource, value, group = mod_f_respondent_sex))+
  geom_col(aes(fill = mod_f_respondent_sex), position = "dodge")

final%>%
  group_by(mod_f_respondent_sex)%>%
  summarise(across(threats_big_a:threats_big_i, ~ (sum(.x, na.rm = TRUE))))%>%
  pivot_longer(threats_big_a:threats_big_i, names_to = "protect_resource", names_prefix = "protect_")%>%
  ggplot(aes(protect_resource, value, group = mod_f_respondent_sex))+
  geom_col(aes(fill = mod_f_respondent_sex), position = "dodge")

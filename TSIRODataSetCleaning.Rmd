...
final <- reduce(list(roster_merge, agriculture_merge, credit_merge, resources_merge, energy_merge, poverty_merge, foodsec_merge, empower_merge), full_join, by = "hhid_deid")%>%
  ...
  mutate(nrm_pract_sum = rowSums(select(resources_merge, nrm_pract_a:nrm_pract_y)), .after = nrm_pract_g)%>%
...
corrplot(MMV, method = 'color', p.mat = testResMV$p, tl.cex = 0.5, insig = 'label_sig', sig.level = c(0.001, 0.01, 0.05), pch.cex = 0.8, number.cex = 0.5, diag = FALSE, is.corr = FALSE, type = 'lower', col = COL2('BrBG'), cl.pos = 'r', cl.cex = 0.3, tl.col = "black")
